...
/*   Updated: 2024/08/21 16:59:19 by joandre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	place_image(char asset, t_mlx *libx, t_axis axi)
{
	if (asset == '1')
		mlx_put_image_to_window(libx->init, libx->window,
			libx->game.w.img, axi.x * ASSET_SIZE, axi.y * ASSET_SIZE);
	else if (asset == '0')
		mlx_put_image_to_window(libx->init, libx->window,
			libx->game.s.img, axi.x * ASSET_SIZE, axi.y * ASSET_SIZE);
	else if (asset == 'C')
		mlx_put_image_to_window(libx->init, libx->window,
			libx->game.c.img, axi.x * ASSET_SIZE, axi.y * ASSET_SIZE);
	else if (asset == 'E')
		mlx_put_image_to_window(libx->init, libx->window,
			libx->game.e.img, axi.x * ASSET_SIZE, axi.y * ASSET_SIZE);
	else if (asset == 'P')
		mlx_put_image_to_window(libx->init, libx->window, libx->game.p.img,
			libx->game.player.x * ASSET_SIZE,
			libx->game.player.y * ASSET_SIZE);
}
